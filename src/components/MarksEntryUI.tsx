...
export default function MarksEntryUI({
  ...
  const [editableSubjectIds, setEditableSubjectIds] = useState<
    (number | string)[]
  >([]);
  ...
  useEffect(() => {
    if (userRole !== "admin" && !currentTeacherId) {
      Alert.alert(
        "Error",
        "Teacher ID is missing! Parent component එකෙන් ID එක එන්නේ නෑ.",
      );
    }
    fetchFormData();
  }, []);
  ...
  const getFilteredSubjects = () => {
    ...
    if (num >= 6 && num <= 9) {
      ...
      if (gradeName.includes("Commerce")) {
        return subjects.filter((subject) =>
          grade12to13CommerceSubjects.includes(subject.name),
        );
      } else {
        return subjects.filter((subject) =>
          grade12to13ArtsSubjects.includes(subject.name),
        );
      }
    }
  ...
  const handleSubmitMarks = async () => {
    ...
    try {
      ...
      if (response.ok && json.success !== false) {
        ...
        const message = json.message || "Unable to submit marks.";
  ...
  const handleExportExcel = async () => {
    ...
    const url = `${API_URL}/generate-report?grade_id=${selectedGrade.id}&subject_id=${selectedSubject.id}&exam_year_id=${selectedYear.id}&term_id=${selectedTerm.id}&format=xlsx`;
    await WebBrowser.openBrowserAsync(url);
  };
      ...
      >
        ...
        >
          ...
          <View className="rounded-[32px] bg-white p-5 shadow-xl shadow-black/5 mb-6">
            ...
            {!canEditMarks ? (
              <View className="mb-5 rounded-2xl bg-[#fceeed] px-4 py-3 border border-[#f5c3bd]">
                <Text className="text-[12px] font-bold text-[#8f140e]">
                  Read-only view
                </Text>
                <Text className="mt-1 text-[12px] text-[#8f140e]/80 leading-relaxed">
                  You can review only the grades and subjects assigned to your
                  account.
                </Text>
              </View>
            ) : null}
          ...
          <View className="rounded-[32px] bg-white p-5 shadow-xl shadow-black/5 mb-6">
            <View className="gap-3">
              ...
              <Pressable
                ...
                className="h-14 items-center justify-center rounded-full bg-[#fceeed] border border-[#f5c3bd]"
              >
                <View className="flex-row items-center justify-center">
                  <Feather name="download" size={18} color="#8f140e" />
                  <Text className="text-[16px] font-bold text-[#8f140e] ml-2">
                    Export Excel
                  </Text>
                </View>
              </Pressable>
